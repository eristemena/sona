...
export function WordLookupPopup({
  ...
  const popupStyle = useMemo(() => {
    ...
    return {
      left,
      top: shouldFlipAbove ? Math.max(16, rect.top - (POPUP_VERTICAL_SPACE - 52)) : rect.bottom + 12,
      width,
    }
  }, [target])

  const addToDeckLabel = addToDeckResult?.disposition === 'created'
    ? 'Added'
        ...
        >
          <div className="flex items-start justify-between gap-2.5">
            <div className="min-w-0">
              <h3 className="text-[18px] font-semibold text-(--text-primary)" id="word-lookup-title">
                {target?.token ?? lookupResult?.surface ?? 'Selected word'}
              </h3>
              {lookupResult?.romanization ? (
                <p className="mt-1 text-[13px] italic text-(--text-secondary)">{lookupResult.romanization}</p>
              ) : null}
              {!lookupResult && isLoadingLookup ? (
                <p className="mt-1 text-[13px] italic text-(--text-secondary)">Looking up...</p>
              ) : null}
            </div>
            <Button aria-label="Close word lookup" className="h-7 w-7 rounded-md px-0 text-(--text-secondary) hover:text-(--text-primary)" onClick={onDismiss} variant="ghost">
              <X aria-hidden="true" className="h-3.5 w-3.5" />
            </Button>
          </div>

          {isLoadingLookup ? (
            <p aria-live="polite" className="mt-3 text-[15px] text-(--text-primary)">
          ...
          {lookupResult ? (
            <div className="mt-3 space-y-3">
              <p className="text-[15px] text-(--text-primary)">{lookupResult.meaning}</p>
              ...
              <p className="text-[13px] italic leading-5 text-(--text-secondary)">{lookupResult.sentenceTranslation}</p>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                ...
                <Button
                  ...
                  disabled={isAddingToDeck || Boolean(addToDeckResult)}
                  onClick={onAddToDeck}
                  variant="secondary"
                >
              ...
              {isLoadingGrammar ? (
                ...
                <div className="space-y-3 border-t border-(--border) pt-3">
                  <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-(--text-muted)">Grammar note</p>
                  <p className="text-[14px] leading-[1.6] text-(--text-primary)">{lookupResult.grammarExplanation}</p>
                </div>
              ) : null}
