...
export function ReviewCardBack({ card, isMarkingKnown, isSavingDetails, onMarkKnown, onSaveDetails }: ReviewCardBackProps) {
  const [isEditing, setIsEditing] = useState(false)
  const [meaning, setMeaning] = useState(card.back.meaning ?? '')
  const [grammarPattern, setGrammarPattern] = useState(card.back.grammarPattern ?? '')
  const [grammarDetails, setGrammarDetails] = useState(card.back.grammarDetails ?? '')
    ...
    return (
      <div className="space-y-4 rounded-[1.25rem] border border-(--border) bg-[color-mix(in_srgb,var(--accent)_8%,transparent)] p-4 panel-enter">
        <div className="space-y-2">
          <label className="block text-xs font-medium uppercase tracking-[0.22em] text-(--text-muted)" htmlFor="review-card-meaning">
            Meaning
          </label>
          <textarea
        ...
        <div className="space-y-2">
          <label className="block text-xs font-medium uppercase tracking-[0.22em] text-(--text-muted)" htmlFor="review-card-grammar-pattern">
            Grammar pattern
          </label>
          <input
        ...
        <div className="space-y-2">
          <label className="block text-xs font-medium uppercase tracking-[0.22em] text-(--text-muted)" htmlFor="review-card-grammar-details">
            Grammar details
          </label>
          <textarea
            className="min-h-24 w-full rounded-xl border border-(--border) bg-(--bg-surface) px-3 py-2 text-sm text-(--text-primary) outline-none focus:border-(--accent)"
        ...
        <div className="flex justify-end gap-3">
          <Button disabled={isSavingDetails} onClick={() => setIsEditing(false)} type="button" variant="ghost">
            Cancel
          </Button>
          <Button disabled={isSavingDetails} onClick={() => void handleSave()} type="button" variant="primary">
