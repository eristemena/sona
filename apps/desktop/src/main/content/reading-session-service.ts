...
import { KnownWordService } from "./known-word-service.js";
...
export class ReadingSessionService {
  ...
  getReadingSession(contentItemId: string): ReadingSessionSnapshot {
    ...
    if (!session) {
      throw new Error(
        `Reading session content was not found for ${contentItemId}.`,
      );
    }
  ...
  async explainGrammar(
    ...
    return this.options.annotationCacheService.explainGrammar(input, {
      canonicalCandidate:
        input.canonicalForm ?? this.resolveCanonicalCandidate(input),
    });
  }
  ...
  getWordStudyStatus(input: { canonicalForm: string; surface: string }) {
    return this.options.knownWordService.getWordStudyStatus(input);
  }
  ...
  private resolveCanonicalCandidate(
    input: Pick<WordLookupInput, "blockId" | "tokenIndex">,
  ): string | null {
