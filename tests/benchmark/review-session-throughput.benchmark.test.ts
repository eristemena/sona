...
function createRepository() {
  const directory = mkdtempSync(path.join(tmpdir(), 'sona-review-throughput-benchmark-'))
...
function seedReviewSource(repository: SqliteContentLibraryRepository, index: number, now: number) {
  ...
  repository.saveContent({
    item: {
      id: contentItemId,
      title: `Review throughput item ${index + 1}`,
      sourceType: 'article',
      difficulty: 1,
      difficultyLabel: 'Beginner',
      provenanceLabel: 'Article paste',
      sourceLocator: `seed://review-throughput-${index + 1}`,
      provenanceDetail: 'Seeded for review throughput benchmark coverage.',
      searchText: `review throughput item ${index + 1}`,
    ...
    sourceRecord: {
      ...
      filePath: null,
      url: null,
      sessionId: `seed-review-throughput-${index + 1}`,
      displaySource: 'Seeded for review throughput benchmark coverage.',
...
describe('review session throughput benchmark', () => {
  it('keeps a 20-card rating run under the 6-minute acceptance budget', () => {
    const now = 1_716_740_000_000
    ...
    const queueAfterRun = service.getQueue()

    expect(queueAfterRun.dueCount).toBe(0)
    expect(queueAfterRun.cards).toHaveLength(0)
