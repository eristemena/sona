...
describe('generated content difficulty validation', () => {
  ...
  it('saves accepted generated content and warns before saving a duplicate', async () => {
    ...
    const firstResult = await generateHandler(undefined, { topic: 'coffee shop', sentenceCount: 10, difficulty: 2 })
    ...
    const duplicateWarning = await generateHandler(undefined, { topic: 'coffee shop', sentenceCount: 10, difficulty: 2 })
  ...
  it('relabels generated content and preserves requested versus validated difficulty provenance', async () => {
    ...
    const generator: PracticeSentenceGenerator = {
      async generateSentences() {
        return {
          sentences: [
            "시청역으로 가려면 여기에서 2호선으로 갈아타세요.",
            "출구를 나오면 큰 은행 건물이 바로 보여요.",
          ],
        };
      },
      async validateDifficulty() {
    ...
    const result = await generateHandler(undefined, { topic: 'subway directions', sentenceCount: 10, difficulty: 2 })
  ...
  expect(savedItem?.title).toBe("Subway directions");
  ...
  it('rejects generated content that fails difficulty validation without saving a partial item', async () => {
    ...
    const generator: PracticeSentenceGenerator = {
      async generateSentences() {
        return {
          sentences: [
            "이 제도는 장기적으로 시민 참여를 재구성할 가능성이 있습니다.",
          ],
        };
      },
      async validateDifficulty() {
    ...
    const result = await generateHandler(undefined, { topic: 'debate club', sentenceCount: 10, difficulty: 1 })
  ...
  it("ignores an LLM-supplied Korean title and stores the capitalized topic instead", async () => {
    const database = createSqliteConnection({ databasePath: ":memory:" });
    runShellMigrations(database);
    const repository = new SqliteContentLibraryRepository(database);
    const generator: PracticeSentenceGenerator = {
      async generateSentences() {
        return {
          sentences: ["주문하신 음료는 오른쪽에서 받아 가세요."],
        };
      },
      async validateDifficulty() {
        return {
    ...
    registerContentHandlers(
      {
        contentRepository: repository,
        generatedContentService: new GeneratedContentService(generator, {
    ...
    expect(repository.listLibraryItems()[0]?.title).toBe("Ordering coffee");
  });
})