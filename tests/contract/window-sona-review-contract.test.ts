...
describe('window.sona review preload contract', () => {
  ...
  it('exposes the typed review surface and invokes the review channels', async () => {
    ...
    expect(electronMockState.ipcRenderer.invoke).toHaveBeenNthCalledWith(2, 'sona:review:submit-rating', {
      reviewCardId: 'card-1',
      rating: 'good',
    })
    expect(electronMockState.ipcRenderer.invoke).toHaveBeenNthCalledWith(3, 'sona:review:update-card-details', {
      reviewCardId: 'card-1',
      meaning: 'slowly',
      grammarPattern: 'Adverbial pacing',
      grammarDetails: 'Used to soften reading pace.',
    })
    expect(electronMockState.ipcRenderer.invoke).toHaveBeenNthCalledWith(4, 'sona:review:get-known-word-onboarding-status')
    expect(electronMockState.ipcRenderer.invoke).toHaveBeenNthCalledWith(5, 'sona:review:complete-known-word-onboarding', {
      seedPackId: 'topik-i-core',
      selectedWords: [{ canonicalForm: '천천히', surface: '천천히' }],
    })
    expect(electronMockState.ipcRenderer.invoke).toHaveBeenNthCalledWith(6, 'sona:review:mark-known-word', {
      canonicalForm: '천천히',
      surface: '천천히',
      source: 'manual',
      sourceDetail: 'Learner already owns this word.',
    ...
    expect(electronMockState.ipcRenderer.invoke).toHaveBeenNthCalledWith(7, 'sona:review:clear-known-word', {
      canonicalForm: '천천히',
      reviewCardId: 'card-1',
    })
    expect(electronMockState.ipcRenderer.invoke).toHaveBeenNthCalledWith(8, 'sona:reading:get-word-study-status', {
      canonicalForm: '천천히',
      surface: '천천히',
    })
  })
