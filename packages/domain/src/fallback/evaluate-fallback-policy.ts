import type {
  LlmFallbackArtifact,
  LlmFallbackPolicyArtifact,
} from "../artifacts/spike-artifact-schema.js";
...
export function evaluateFallbackPolicies(
  ...
  const artifactPolicies: LlmFallbackPolicyArtifact[] = policies.map(
    (policy) => ({
...
function providerOptionsFor(
  featureArea: ProviderFallbackPolicy["featureArea"],
): string[] {
