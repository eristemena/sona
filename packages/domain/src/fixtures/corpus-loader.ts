...
export async function loadCorpusSegments(
  rootDir = path.join(process.cwd(), "fixtures", "corpus"),
): Promise<CorpusSegment[]> {
  ...
  const segments = await Promise.all(
    files.map(async (filePath) => {
      ...
      return Array.isArray(raw)
        ? raw.map(validateCorpusSegment)
        : [validateCorpusSegment(raw)];
    }),
  );
