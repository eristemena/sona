...
export async function runSqliteConcurrencySpike(
  options: RunSqliteConcurrencySpikeOptions = {},
) {
  ...
  try {
    ...
    const report = validateSqliteConcurrencyReport({
      ...
      bottlenecks:
        durationMs <= 30000 ? [] : ["throughput-above-target-window"],
      mitigations:
        durationMs <= 30000
          ? []
          : ["Reduce batch size or checkpoint frequency."],
    });
