...
**Performance Goals**: Reading view becomes usable immediately on open, cached block replay starts without provider wait, uncached audio generation degrades after the existing 10-second TTS budget, lookup remains instant on cache hits, and highlight drift stays within the spoken token or an adjacent token during validation  
**Constraints**: No accounts, no telemetry, optional network only for TTS and lookup enrichment, provider credentials confined to main process, no per-word real-time SQLite writes during playback, manual add-to-deck only, and clear fallback when timing payloads or provider responses are unavailable  
...
- **Reading and listening integration**: PASS. Text, audio, and review capture are unified in a single reading session. Synced highlighting follows the active audio asset when available, and the same reading surface still supports lookup and deck capture when audio is missing.
...
- **Learner-owned content**: PASS. `review_cards` store source block and content item provenance, `annotations` keep full structured responses inspectable, and `block_audio_assets` are keyed to learner-owned block text rather than opaque document-level jobs.
...
- **Reading and listening integration**: PASS. `ReadingSessionSnapshot` hydrates text, progress, and block state together; `BlockAudioAsset` persists timing data for karaoke sync; and text-first reading remains the fallback when audio generation or timestamps are unavailable.
