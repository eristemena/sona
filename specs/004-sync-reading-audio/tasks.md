...
**Goal**: Open saved content in a focused reading surface with block-level audio generation, cached replay, and karaoke-style word highlighting.

**Independent Test**: Open a saved content item, confirm the reading text appears immediately, start playback, verify highlighted words follow the spoken token through pause, replay, speed changes, and scrubbing, then reopen the same content and confirm cached audio avoids a second provider wait.
...
- [ ] T035 [US3] Add review-capture and exposure-log manual verification notes to /Volumes/xpro/erisristemena/made-by-ai/sona/specs/004-sync-reading-audio/quickstart.md
...
- [ ] T036 [P] Add regression coverage for reading-progress restore and cached-audio cleanup on invalidation in /Volumes/xpro/erisristemena/made-by-ai/sona/tests/integration/reading-progress-persistence.test.tsx and /Volumes/xpro/erisristemena/made-by-ai/sona/tests/integration/audio-cache-cleanup.test.ts
...
- Persistence, audio generation, annotation caching, and review scheduling require tests before implementation.
